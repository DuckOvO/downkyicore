...
- Runtime code must not rebuild a Domain task from a mutable UI projection. `DownloadTask.Restore` is limited to SQLite materialization; legacy history migration uses the completion-specific `DownloadTask.ImportCompletedHistory` factory, and architecture tests enforce both allowlists.
